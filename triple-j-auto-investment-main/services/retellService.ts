...
interface VehicleInquiryPayload {
  ...
  // Phase 17: Rental inquiry context
  daily_rate?: string;
  weekly_rate?: string;
  // Phase 17: Language preference for bilingual support
...
export async function triggerOutboundCall(payload: VehicleInquiryPayload): Promise<RetellCallResponse> {
  ...
  try {
    // Detect rental inquiries for context-aware Retell agent prompts
    const isRentalInquiry = payload.inquiry_source === 'rental_inquiry' ||
                            payload.inquiry_source === 'rental' ||
                            payload.inquiry_source === 'Website Rental Inquiry';
    ...
    const response = await fetch('https://api.retellai.com/v2/create-phone-call', {
      ...
      body: JSON.stringify({
        ...
        retell_llm_dynamic_variables: {
          ...
          // Phase 17: Rental context for Retell agent prompt
